...
if __name__ == "__main__":
    ...
    #                     spike_times[name] += torch.sum(
    #                         torch.from_numpy(np.concatenate(module.monitor['s'], axis=0)).cuda(), dim=0)
    #
    #             reset_net(net)
    #
    ...
    #
    #             print(
    #                 f'Test Acc: {test_accuracy * 100:.3f}%, Sparsity (w/ BN): {sparsity_all * 100:.3f}%, Sparsity (w/o BN): {sparsity_w * 100:.3f}%')
    #
    # ###### TRAIN MODE ######
    # else:
    ...
    #                 optimizer_w = GradRewiring(weight_params, lr=learning_rate, alpha=penalty, s=s)
    #             elif args.mode == 'CCEP':
    #                 optimizer_w = CCEP(weight_params, lr=learning_rate, alpha=penalty, s=s)
    #             elif args.mode == 'deep':
    #                 optimizer_w = DeepRewiring(weight_params, lr=learning_rate, l1=penalty, max_s=s, soft=soft)
    #             optimizer_bn = Adam(bn_params, lr=learning_rate)
    #
    ...
    #
    #                         regrow_cnt = torch.logical_and(torch.logical_not(before_link[name]),
    #                                                        after_link[name]).sum().item()
    #                         prune_cnt = torch.logical_and(torch.logical_not(after_link[name]),
    #                                                       before_link[name]).sum().item()
    #
    ...
    #                         if net.epochs % i1 == 0 and (
    #                                 name == 'static_conv.1.weight' or name == 'conv.1.weight' or name == 'fc.2.weight'):
    ...
    #                                 torch.save(state['strength'],
    #                                            os.path.join(model_dir, f'{name}-theta-{net.epochs}.pkl'))
    #                 ######################################################################
    #
    #                 sparsity_all = zero_cnt / ttl_cnt
    #                 sparsity_w = zero_cnt / w_cnt
